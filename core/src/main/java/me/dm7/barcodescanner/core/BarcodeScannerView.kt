...
abstract class BarcodeScannerView : FrameLayout, Camera.PreviewCallback {
    ...
    fun setupLayout(cameraWrapper: CameraWrapper) {
        ...
        mPreview = CameraPreview(context, cameraWrapper, this).apply {
            setAspectTolerance(mAspectTolerance)
            setShouldScaleToFill(mShouldScaleToFill)
        }
    ...
    fun getRotationCount(): Int {
        val displayOrientation = mPreview?.getDisplayOrientation()
        return displayOrientation ?: 0 / 90
    }
}